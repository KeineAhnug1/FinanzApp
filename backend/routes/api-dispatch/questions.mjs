...
export async function dispatchQuestionRoutes(ctx) {
  ...
  if (questionAnswerMatch)
    return await handlers.handleQuestionAnswerCreate(request, questionAnswerMatch[1], session);

  const questionLikeMatch = pathname.match(/^\/api\/questions\/([^/]+)\/like$/);
  if (questionLikeMatch)
    return await handlers.handleQuestionLike(request, questionLikeMatch[1], session);

  const questionByIdMatch = pathname.match(/^\/api\/questions\/([^/]+)$/);
  if (questionByIdMatch)
    return await handlers.handleQuestionById(request, questionByIdMatch[1], session);

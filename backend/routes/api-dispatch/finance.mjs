...
export async function dispatchFinanceRoutes(ctx) {
  ...
  if (pathname === "/api/share-accounts")
    return await handlers.handleShareAccounts(request, session);
  const shareAccountId = parsePathParam(pathname, "/api/share-accounts/");
  if (shareAccountId)
    return await handlers.handleShareAccountById(request, shareAccountId, session);
  if (pathname === "/api/debug/positions")
    return await handlers.handleDebugPositions(request, url, session);
  if (pathname.startsWith("/api/twelvedata"))
    return await handlers.handleTwelveDataProxy(request, pathname, url, session);
  if (pathname === "/api/stocks/search")
    return await handlers.handleStockSearchProxy(request, url, session);
  if (pathname === "/api/stocks/logo")
    return await handlers.handleStockLogoProxy(request, url, session);

...
export async function dispatchEntryRoutes(ctx) {
  ...
  if (pathname === "/api/income-entries") return await handlers.handleIncomeEntries(request, session);
  ...
  if (pathname === "/api/expense-entries") return await handlers.handleExpenseEntries(request, session);
  const expenseEntryId = parsePathParam(pathname, "/api/expense-entries/");
  if (expenseEntryId) return await handlers.handleExpenseEntryById(request, expenseEntryId, session);

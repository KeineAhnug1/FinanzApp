...
export async function dispatchApiRoute(ctx) {
  let result;
  result = await dispatchUserRoutes(ctx);
  if (result) return result;
  result = await dispatchFinanceRoutes(ctx);
  if (result) return result;
  result = await dispatchBudgetRoutes(ctx);
  if (result) return result;
  result = await dispatchGroupRoutes(ctx);
  if (result) return result;
  result = await dispatchQuestionRoutes(ctx);
  if (result) return result;
  result = await dispatchEntryRoutes(ctx);
  if (result) return result;

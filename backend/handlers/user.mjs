...
export function createUserHandlers({ pool, kv, destroySession, clearSessionCookie }) {
  ...
  async function handleDeleteUserAccount(request, session) {
    ...
    try {
      ...
      await client.query(
        `DELETE FROM income WHERE bank_account_id IN (SELECT id FROM bank_accounts WHERE user_id = $1)`,
        [userId]
      );
      await client.query(
        `DELETE FROM private_expenses WHERE bank_account_id IN (SELECT id FROM bank_accounts WHERE user_id = $1)`,
        [userId]
      );
      ...
      await client.query(
        `DELETE FROM email_verifications WHERE email IN (SELECT email FROM users WHERE id = $1)`,
        [userId]
      );
  ...
  async function handlePasswordChange(request, session) {
    ...
    if (!currentPassword || !newPassword)
      return badRequest("Aktuelles und neues Passwort sind Pflichtfelder");
    ...
    if (!isValid)
      return jsonResponse(
        { ok: false, code: "wrong_password", message: "Aktuelles Passwort ist falsch" },
        400
      );
  ...
  async function handleProfileImageUpload(request, session) {
    if (request.method !== "PUT")
      return jsonResponse({ ok: false, message: "Method not allowed" }, 405, { Allow: "PUT" });
    const rl = checkRateLimit(request, {
      maxAttempts: 10,
      windowMs: 60_000,
      group: "profile-image",
    ...
    if (!profileImage || typeof profileImage !== "string")
      return badRequest("profileImage ist ein Pflichtfeld");

...
export function createGroupHandlers(pool) {
  ...
  async function getGroupContext(groupIdRaw, sessionUserId) {
    ...
    const [userResult, groupResult] = await Promise.all([
      pool.query(`SELECT id, username, first_name, last_name FROM users WHERE id = $1`, [
        userObjectId,
      ]),
  ...
  async function deleteGroupCascade(groupId) {
    ...
    if (fundingIds.length) {
      await Promise.all([
        pool.query(`DELETE FROM funding_participants WHERE group_funding_id = ANY($1)`, [
          fundingIds,
        ]),
  ...
  async function handleGroupDetail(request, groupIdRaw, session) {
    ...
    if (!context.ok)
      return jsonResponse(
        { ok: false, message: context.message },
        /** @type {number} */ (context.status)
      );
    ...
    return jsonResponse(
      {
        ok: true,
        group: {
          group_id: String(context.group.id),
        ...
        fundings: fundings.map((f) => {
          const linkedActivity = f.group_activity_id
            ? activityById.get(String(f.group_activity_id))
            : null;
          ...
          return {
            ...
            amount: toNullableNumber(f.amount),
            info: f.info ?? null,
            created_at: f.created_at ?? null,
            contributions: contributions.map((/** @type {any} */ c) => ({
              user_id: String(c.user_id),
              username: c.username,
              first_name: c.first_name ?? null,
              last_name: c.last_name ?? null,
              amount: toNullableNumber(c.amount),
              ...
              contributions
                .reduce(
                  (/** @type {number} */ sum, /** @type {any} */ c) =>
                    sum + (toNullableNumber(c.amount) ?? 0),
                  0
                )
                .toFixed(2)
            ),
            linked_activity: linkedActivity
              ? {
                  activity_id: String(linkedActivity.id),
        ...
        expenses: expenses.map((e) => {
          ...
          return {
            group_expense_id: String(e.id),
            group_funding_id: String(e.group_funding_id),
            funding_info: f?.info ?? null,
            amount: toNullableNumber(e.amount),
            info: e.info ?? null,
            state: e.state ?? null,
            cycle: e.cycle ?? null,
            due_date: e.due_date ?? e.pay_date ?? null,
            pay_date: e.pay_date ?? e.due_date ?? null,
        ...
        funding_transactions: transactions.map((t) => {
          ...
          return {
            ...
            group_expense_id: String(t.group_expense_id),
            group_funding_id: e ? String(e.group_funding_id) : null,
            amount: e ? toNullableNumber(e.amount) : null,
            created_at: t.created_at ?? null,
            expense_info: e?.info ?? null,
            funding_info: f?.info ?? null,
          };
        }),
      },
    ...
    return jsonResponse(
      {
        ok: true,
        activity: {
          activity_id: String(insertResult.rows[0].id),
          info,
          date,
  ...
  async function handleCreateGroupFunding(request, groupIdRaw, session) {
    ...
    if (activityIdRaw) {
      ...
      if (linkedResult.rows.length === 0)
        return badRequest("Linked activity does not exist in this group");
    }
    ...
    const insertResult = await pool.query(
      ...
      [/** @type {number} */ (context.groupId), groupActivityId, 0.0, info, createdAt]
    );
    return jsonResponse(
      {
        ok: true,
        funding: {
          funding_id: String(insertResult.rows[0].id),
  ...
  async function handleDonateToFunding(request, groupIdRaw, fundingIdRaw, session) {
    ...
    if (normalizedAmount > currentBalance)
      return badRequest(
        "Not enough available balance based on your dashboard entries for this donation"
      );
    ...
    try {
      ...
      if (existingResult.rows.length > 0) {
        ...
        await client.query(`UPDATE funding_participants SET amount = $1 WHERE id = $2`, [
          toDecimal(nextAmount),
          existing.id,
        ]);
      } else {
      ...
      await client.query(`UPDATE group_funding SET amount = $1 WHERE id = $2`, [
        toDecimal(updatedFundingAmount),
        fundingId,
      ]);
      ...
      return jsonResponse(
        {
          ...
          donation: {
            ...
            bank_balance: Number((currentBalance - normalizedAmount).toFixed(2)),
          },
        },
        201
      );
  ...
  async function handleCreateGroupExpense(request, groupIdRaw, session) {
    ...
    if (context.membership.role !== "admin")
      return forbidden("Only admins can create group expenses");
    ...
    if ((payload.due_date || payload.pay_date) && !payDate)
      return badRequest("Expense due date is invalid");

    const info = String(payload.info || "").trim() || null;
    const fundingBalance = toNullableNumber(funding.amount) ?? 0;
    if (normalizedAmount > fundingBalance)
      return badRequest("Funding balance is too low for this expense");
    ...
    await pool.query(`INSERT INTO transactions (group_expense_id, created_at) VALUES ($1, $2)`, [
      expenseResult.rows[0].id,
      createdAt,
    ]);

    const updatedFundingBalance = Number((fundingBalance - normalizedAmount).toFixed(2));
    await pool.query(`UPDATE group_funding SET amount = $1 WHERE id = $2`, [
      toDecimal(updatedFundingBalance),
      fundingId,
    ]);
    ...
    return jsonResponse(
      {
        ok: true,
        expense: {
          group_expense_id: String(expenseResult.rows[0].id),
  ...
  async function handleGroupMessages(request, groupIdRaw, session) {
    ...
    if (request.method === "GET") {
      ...
      const limit = Number.isFinite(requestedLimit)
        ...
        : 30;
      const beforeMessageIdRaw = String(
        requestUrl.searchParams.get("before_message_id") || ""
      ).trim();
      ...
      if (beforeMessageIdRaw) {
        ...
        if (beforeResult.rows.length === 0)
          return notFound("Cursor message not found in this group");
      ...
      const messages = orderedRows.map((entry) => {
        ...
        return {
          ...
          group_id: String(/** @type {number} */ (context.groupId)),
    ...
    if (detectBlockedMessageTerm(message))
      return badRequest(
        "Die Nachricht enthaelt verbotene Begriffe und kann nicht gesendet werden."
      );
    ...
    return jsonResponse(
      {
        ok: true,
        message: {
          message_id: String(insertResult.rows[0].id),
          group_id: String(/** @type {number} */ (context.groupId)),
          from_user_id: String(context.user.id),
  ...
  async function handleInviteUser(request, groupIdRaw, session) {
    ...
    if (existingResult.rows.length > 0) {
      ...
      if (existingMembership.status === "denied") {
        await pool.query(`UPDATE group_members SET role = $1, status = $2 WHERE id = $3`, [
          "member",
          "invited",
      ...
      if (existingMembership.status === "invited")
        return conflict("User already has a pending invitation");
  ...
  async function handleGetInvitations(request, session) {
    ...
    return jsonResponse(
      {
        ok: true,
        invitations: result.rows.map((e) => ({
          group_id: String(e.group_id),
          group_name: e.group_name,
          group_address: e.group_address ?? null,
          group_created_at: e.group_created_at ?? null,
          role: e.role,
          status: e.status,
        })),
      },
  ...
  async function handleInvitationDecision(request, groupIdRaw, decision, session) {
    ...
    if (decision !== "accept" && decision !== "deny")
      return badRequest("Invalid invitation decision");
  ...
  async function handleRemoveMember(request, groupIdRaw, userIdRaw, session) {
    if (request.method !== "DELETE")
      return jsonResponse({ ok: false, message: "Method not allowed" }, 405, { Allow: "DELETE" });
    const context = await getGroupContext(groupIdRaw, session.user.id);
    if (!context.ok)
      return jsonResponse(
        { ok: false, message: context.message },
    ...
    if (context.membership.role !== "admin")
      return forbidden("Only admins can remove participants");
    ...
    if (targetUserId === context.user.id)
      return badRequest("You can only remove other participants");
  ...
  async function handlePromoteMemberToAdmin(request, groupIdRaw, userIdRaw, session) {
    ...
    await pool.query(`UPDATE group_members SET role = $1 WHERE id = $2`, [
      "admin",
      targetMembership.id,
    ]);
  ...
  async function handleLeaveGroup(request, groupIdRaw, session) {
    ...
    const leaveResult = await pool.query(`DELETE FROM group_members WHERE id = $1`, [
  ...
  async function handleDeleteGroupMessage(request, groupIdRaw, messageIdRaw, session) {
    ...
    if (existing.from_user_id !== userId)
      return forbidden("Nur der Absender darf diese Nachricht löschen");
    if (existing.deleted_at)
      return jsonResponse({ ok: false, message: "Nachricht wurde bereits gelöscht" }, 400);

    await pool.query(`UPDATE group_message SET message = NULL, deleted_at = NOW() WHERE id = $1`, [
      messageId,
    ]);

...
export function createBudgetHandlers(pool) {
   ...
   */
  async function handleBudgets(request, session) {
    ...
    if (targetAmount == null) return badRequest("Zielbetrag muss groesser 0 sein");
   ...
   * @param {{ user: { id: string } }} session
   */
  async function handleBudgetById(request, budgetIdRaw, session) {
    const budgetId = parseId(budgetIdRaw);
    if (!budgetId) return badRequest("budget_id ist ungueltig");
  ...
  async function handleBudgetStatus(request, session) {
    ...
    if (budgets.length === 0) return jsonResponse({ ok: true, alerts: [] }, 200);
    ...
    if (accountIds.length > 0) {
      ...
      const { rows: expenses } = await pool.query(
        `SELECT LOWER(category) AS cat, SUM(amount) AS total
         ...
         WHERE bank_account_id = ANY($1) AND spent_at >= $2 AND category IS NOT NULL

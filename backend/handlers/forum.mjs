    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        if (!response.ok) {
          ...
          console.error(
            `OpenRouter request failed (key #${index + 1}, attempt ${attempt + 1}):`,
            detail
          );
        ...
        console.error(
          `OpenRouter request crashed (key #${index + 1}, attempt ${attempt + 1}):`,
          err
        );
        await new Promise((r) => setTimeout(r, backoffMs));
        backoffMs = Math.min(backoffMs * 2, 4000);
...
export async function generateFinzbroAnswer(thema, message) {
  ...
  if (!res.ok)
    return "Es gibt momentan leider Probleme mit dieser AI. Bitte später erneut versuchen.";
...
export function createForumHandlers(pool) {
  ...
  async function fetchSingleQuestionWithRelations(userId, questionId) {
    ...
    const [usersResult, questionLikeRows, answerLikeRows, likedQuestionsByMe, likedAnswersByMe] =
      await Promise.all([
        pool
          .query(`SELECT id, username, first_name FROM users WHERE id = ANY($1)`, [
            Array.from(userIdSet.values()),
          ])
          .then((r) => r.rows),
        pool
          .query(`SELECT COUNT(*)::int AS count FROM question_likes WHERE question_id = $1`, [
            question.id,
          ])
          .then((r) => r.rows),
        answerIds.length
          ...
          : Promise.resolve([]),
        pool
          .query(`SELECT question_id FROM question_likes WHERE user_id = $1 AND question_id = $2`, [
            userId,
            question.id,
          ])
          .then((r) => r.rows),
    ...
    const likesCountByQuestionId = new Map([
      [String(question.id), questionLikeRows[0]?.count || 0],
    ]);
    const answerLikesCountByAnswerId = new Map(
      answerLikeRows.map((r) => [String(r.answer_id), r.count])
    );
  ...
  async function listQuestionsWithRelations(userId, searchRaw = "") {
    ...
    const questions = hasSearch
      ? (() => {
          const scored = candidateQuestions.map((q) => ({
            question: q,
            ...scoreQuestionTitle(q?.thema || "", searchTokens),
          }));
          const strictMatches = scored.filter((e) => e.allMatched);
          const fallbackMatches =
            strictMatches.length > 0 ? strictMatches : scored.filter((e) => e.anyMatched);
          fallbackMatches.sort((l, r) => {
            if (r.score !== l.score) return r.score - l.score;
            return (
              new Date(r.question?.created_at || 0).getTime() -
              new Date(l.question?.created_at || 0).getTime()
            );
          });
    ...
    const answers = questionIds.length
      ? (
          await pool.query(
            `SELECT id, question_id, from_user_id, message, edited, created_at, updated_at FROM global_answers WHERE question_id = ANY($1) ORDER BY created_at ASC`,
            [questionIds]
          )
        ).rows
      : [];
    ...
    const users = userIdSet.size
      ? (
          await pool.query(`SELECT id, username, first_name FROM users WHERE id = ANY($1)`, [
            Array.from(userIdSet.values()),
          ])
        ).rows
      : [];
    ...
    const questionLikeRows = questionIds.length
      ? (
          await pool.query(
            `SELECT question_id, COUNT(*)::int AS count FROM question_likes WHERE question_id = ANY($1) GROUP BY question_id`,
            [questionIds]
          )
        ).rows
      : [];
    const likesCountByQuestionId = new Map(
      questionLikeRows.map((r) => [String(r.question_id), r.count])
    );
  ...
  async function handleQuestions(request, session, url) {
    ...
    if (!thema)
      return badRequest(`Thema ist erforderlich (maximal ${QUESTION_TOPIC_MAX_LENGTH} Zeichen).`);
  ...
  async function handleQuestionById(request, questionIdRaw, session) {
    ...
    if (request.method === "DELETE") {
      ...
      if (String(existingRows[0].from_user_id) !== String(userId))
        return forbidden("Nur der Ersteller darf diese Frage loeschen");
    ...
    if (existingRows.length === 0) return notFound("Frage nicht gefunden");
    if (String(existingRows[0].from_user_id) !== String(userId))
      return forbidden("Nur der Ersteller darf diese Frage bearbeiten");
  ...
  async function handleAnswerById(request, answerIdRaw, session) {
    ...
    if (String(answer.from_user_id) !== String(userId))
      return forbidden("Nur der Ersteller darf diese Antwort bearbeiten oder loeschen");

    if (request.method === "DELETE") {
      await pool.query(`DELETE FROM answer_likes WHERE answer_id = $1`, [answerId]);
      await pool.query(`DELETE FROM global_answers WHERE id = $1 AND from_user_id = $2`, [
        answerId,
        userId,
      ]);
      ...
      return jsonResponse(
        { ok: true, question: updatedQuestion, message: "Antwort geloescht" },
        200
      );
    }
    ...
    await pool.query(
      `UPDATE global_answers SET message = $1, edited = true, updated_at = NOW() WHERE id = $2 AND from_user_id = $3`,
  ...
  async function handleQuestionLike(request, questionIdRaw, session) {
    ...
    return jsonResponse(
      { ok: true, question_id: String(questionId), liked, likes_count: countRows[0]?.count || 0 },
      200
    );
  }
  ...
  async function handleAnswerLike(request, answerIdRaw, session) {
    ...
    const { rows: answerRows } = await pool.query(`SELECT id FROM global_answers WHERE id = $1`, [
      answerId,
    ]);
    if (answerRows.length === 0) return notFound("Antwort nicht gefunden");
    ...
    return jsonResponse(
      { ok: true, answer_id: String(answerId), liked, likes_count: countRows[0]?.count || 0 },
      200
    );
  }

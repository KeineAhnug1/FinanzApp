...
auth.post('/register', async (c) => {
  ...
  try {
    const result = await sendVerificationEmail(c.env, email, firstName, code);
    if (result.ok) {
      delivered = true;
    } else if (result.reason === 'send_failed') {
      lastError = `Mail-Provider ${result.status}: ${result.body.slice(0, 200)}`;
    }
  } catch (err) {
    lastError = err instanceof Error ? err.message : String(err);
    console.error('[register] Email send threw:', err);
  }

  const devBypass = c.env.DEV_EXPOSE_VERIFICATION_CODE === 'true' && c.env.NODE_ENV !== 'production';

  if (!delivered && !devBypass) {
    console.error('[register] Email not delivered and dev bypass disabled.', { email, lastError });
    return jsonResponse({
      ok: false,
      message: lastError
        ? `E-Mail konnte nicht versendet werden (${lastError}). Bitte später erneut versuchen.`
        : 'E-Mail-Versand ist nicht konfiguriert. Bitte Administrator kontaktieren.',
    }, 503);
  }
  ...
  return jsonResponse({
    ...
    message: delivered
      ...
      : 'E-Mail-Versand ist im Dev-Modus deaktiviert — der Code wird direkt angezeigt.',
    ...(devBypass && !delivered ? { dev_code: code } : {}),
  }, 200);
});
...
auth.post('/forgot-password', async (c) => {
  ...
  if (user) {
    ...
    let delivered = false;
    try {
      const result = await sendPasswordResetEmail(c.env, email, user.first_name, code);
      delivered = result.ok;
      if (!result.ok && result.reason === 'send_failed') {
        console.error('[forgot-password] Email send failed:', result.status, result.body);
      }
    } catch (err) {
    ...
    if (c.env.NODE_ENV !== 'production') {
      console.log(`[forgot-password:dev] Reset code for ${email}: ${code}`);
    }
    if (!delivered && c.env.DEV_EXPOSE_VERIFICATION_CODE === 'true' && c.env.NODE_ENV !== 'production') {
      devCode = code;
    }
  }

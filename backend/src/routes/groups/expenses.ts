...
// darf target_amount nicht überschreiten.
expenses.post('/:id/funding/:fundingId/expenses', async (c) => {
  ...
  if (funding.status !== 'completed') return badRequest('Sammelaktion noch nicht abgeschlossen — Ausgaben sind erst nach Zielerreichung möglich.');
...
expenses.patch('/:id/funding/:fundingId/expenses/:expenseId', async (c) => {
  ...
  if (oldState !== 'paid' && nextState === 'paid') {
    ...
    const auditDebit = await auth.db.from('transactions').insert({ private_expense_id: privExp.id });
    if ((auditDebit as { error: { message: string } | null }).error) {
      console.error('[expenses.patch] audit insert failed (non-fatal)', (auditDebit as { error: { message: string } }).error);
    }
  }
  // Wechsel von 'paid' zurück: gutschreiben + Pool wieder erhöhen + private_expense löschen
    ...
    if (linkedIds.length) {
      await auth.db.from('transactions').delete().in('private_expense_id', linkedIds);

...
import { EmptyState, IconMessageSquare } from '@/components/ui/EmptyState';
  ...
  return (
    <div className="questions-panel-card questions-panel-card--thread">
      ...
      <div className="answers-section">
        ...
        {(q.answers ?? []).length === 0 && !isLoading ? (
          <EmptyState
            size="sm"
            icon={<IconMessageSquare />}
            title="Noch keine Antworten"
            description="Sei die erste Person, die antwortet."
          />
        ) : (
          (q.answers ?? []).map((a) => (
      ...
      <div className="questions-list-col">
        ...
        {!isLoading && visibleQuestions.length === 0 && (
          search ? (
            ...
            <EmptyState
              icon={<IconMessageSquare />}
              title="Du hast noch keine Fragen gestellt"
              description="Stelle deine erste Frage an die Community."
              cta={{ label: 'Frage stellen', onClick: () => { setSelectedQuestion(null); setShowAskPanel(true); } }}
            />
          ) : (
            <EmptyState
              icon={<IconMessageSquare />}
              title="Noch keine Fragen"
              description="Stelle eine Frage an die Community oder an die KI-Assistenz."
              cta={{ label: 'Frage stellen', onClick: () => { setSelectedQuestion(null); setShowAskPanel(true); } }}
            />
          )
        )}

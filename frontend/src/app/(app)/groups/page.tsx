...
import { EmptyState, IconUsers, IconInbox, IconTarget, IconMessageSquare } from '@/components/ui/EmptyState';
    ...
    <Modal open onClose={onClose} title="Einladungen">
      ...
      {invitations.length === 0 && !isLoading && (
        <EmptyState
          ...
          description="Sobald dich jemand zu einer Gruppe einlädt, erscheint sie hier."
        />
      )}
      {invitations.map((inv) => (
    ...
    <div className="group-detail">
        ...
        <div className="group-section">
          ...
          {(group.members ?? []).length === 0 ? (
            <EmptyState
              ...
              description="Lade jemanden ein, um die Gruppe zu starten."
            />
          ) : (
      ...
      {tab === 'fundings' && (
        <div className="group-section">
          ...
          {(group.funding ?? []).length === 0 && (
            <EmptyState
              size="sm"
              icon={<IconTarget />}
              title="Keine Sammelaktionen"
              description="Starte eine Sammelaktion, um gemeinsam auf ein Ziel zu sparen."
            />
          )}
          {(group.funding ?? []).map((f) => {
          ...
          <div className="chat-messages" ref={chatMessagesRef}>
            {messages.length === 0 ? (
              <EmptyState
                ...
                description="Schreibe die erste Nachricht in der Gruppe."
              />
            ) : (
              messages.map((m) => {
    ...
    <div className="groups-page page-content">
      ...
      {!isLoading && groups.length === 0 && (
        <EmptyState
          ...
          description="Erstelle eine Gruppe, um Ausgaben und Aufgaben gemeinsam zu verwalten."
          cta={{ label: 'Erste Gruppe erstellen', onClick: () => setShowCreate(true) }}
        />
      )}

...
function RegisterForm({
  ...
  onSwitchMode: (mode: AuthMode, email?: string, expiresIn?: number, devCode?: string | null) => void;
}) {
  const {
    register,
    handleSubmit,
  ...
  const onSubmit = async (data: RegisterData) => {
    ...
    const devCode = typeof result.dev_code === 'string' ? result.dev_code : null;
    onSwitchMode('verify', email, expiresIn, devCode);
  };
...
function VerifyForm({
  ...
  return (
    <section className="login-card">
      ...
      {devCode && (
        <div className="dev-code-banner" role="alert">
          <strong>Dev-Modus:</strong> E-Mail-Versand ist nicht konfiguriert. Dein Code lautet:{' '}
          <button
            type="button"
            className="dev-code-banner__code"
            onClick={() => setValue('code', devCode, { shouldValidate: true })}
            aria-label="Code in Feld übernehmen"
          >{devCode}</button>
        </div>
      )}
...
function ForgotForm({
  ...
  const onSubmit = async (data: ForgotData) => {
    ...
    const expiresIn = Number(result.expires_in_seconds) || 0;
    const devCode = typeof result.dev_code === 'string' ? result.dev_code : null;
    onSwitchMode('reset', data.email, expiresIn, devCode);
  };
...
export default function AuthPage() {
  ...
  const switchMode = (nextMode: AuthMode, email?: string, expires?: number, devCode?: string | null) => {
    if (email !== undefined) setPendingEmail(email);
    if (expires !== undefined) setExpiresIn(expires);
    if (devCode !== undefined) setPendingDevCode(devCode);
    else if (nextMode === 'login') setPendingDevCode(null);

import { fnTdFetch, fnT, fnFmtMoney, oShareViewChartUtils } from "./state-api.js";
 ...
 * @returns {string}
 */
export function fnFmtYmd(dDate) {
 ...
 * @returns {number}
 */
export function fnToMsFromTimestamp(nTimestampRaw) {
 ...
 * @returns {number}
 */
export function fnToMsFromTdDatetime(sValueRaw) {
...
export function fnTdSortKey(sValueRaw) {
...
export function fnFmtYmdInTimeZone(iMs, sTimeZone = "America/New_York") {
...
export function fnFmtTdDateTimeInTimeZone(iMs, sTimeZone = "America/New_York") {
  if (!Number.isFinite(iMs)) return "";
...
export function fnIsTdPointOnOrAfterPurchase(sTdDatetime, iPurchaseMs) {
 ...
 * @returns {Date | null}
 */
export function fnGetFirstBuyDate(aPositions) {
 ...
 */
export function fnRangeToQuery(sRange, aPositions) {
  if (sRange === "1D") {
    return {
      interval: "5min",
      outputsize: 576,
 ...
 * @returns {string[]}
 */
export function fnBuildTdSymbolCandidates(sSymbolRaw) {
  const sSymbol = String(sSymbolRaw || "")
    .trim()
    .toUpperCase();
  if (!sSymbol) return [];
  ...
  const fnPushUnique = (sCandidateRaw) => {
    const sCandidate = String(sCandidateRaw || "")
      .trim()
      .toUpperCase();
 ...
 * @returns {object}
 */
export function fnBuildDailyFallbackQuery(oQuery) {
  const oBase = { ...(oQuery || {}) };
  return {
    ...oBase,
    interval: "1day",
 ...
 * @returns {number}
 */
export function fnExtractPositiveQuotePrice(oData) {
 ...
 */
export async function fnGetLatestPriceBySymbolWithFallback(sSymbol) {
 ...
 */
export async function fnLoadSeriesForSymbolWithFallback(sSymbol, oQuery) {
  const aSymbolCandidates = fnBuildTdSymbolCandidates(sSymbol);
  ...
  for (const oCandidateQuery of aQueries) {
    for (const sCandidate of aSymbolCandidates) {
      try {
        ...
        sLastError = fnT("stocks.empty_timeseries_for", "Leere Zeitreihe fuer {symbol}.", {
          symbol: sCandidate,
        });
      } catch (oError) {
        sLastError = String(oError?.message || oError);
 ...
 */
export async function fnLoadSeriesForSymbols(aSymbols, oQuery) {
 ...
 */
export function fnAggregateOwnedSymbols(aPositions) {
  const mBySymbol = new Map();

  for (const oPosition of aPositions || []) {
    const sSymbol = String(oPosition?.symbol || "")
      .trim()
      .toUpperCase();
    if (!sSymbol) continue;
  ...
  const aRows = [...mBySymbol.values()].map((oAgg) => {
    const nAvgBuyPrice =
      oAgg.nTotalAmount > 0 ? oAgg.nWeightedBuySum / oAgg.nTotalAmount : Number.NaN;
    return {
 ...
 */
export function fnBuildSeriesPointsForPositions(aPositions, mSeriesBySymbol, bPnlOnly) {
 ...
 * @returns {number}
 */
export function fnComputeWeightedReturnPct(aPositions, mSeriesBySymbol) {
  ...
  for (const oPosition of aPositions || []) {
    ...
    const aFilteredValues = aSeriesValues.filter((oValue) => {
      ...
      const sPurchaseDay = Number.isFinite(iPurchaseMs)
        ? fnFmtYmdInTimeZone(iPurchaseMs, "America/New_York")
        : "";
 ...
 */
export function fnWithFixedDateRange(aPoints, oQuery, bPnlOnly, sRange = "") {
 ...
 * @returns {string}
 */
export function fnFormatTimeLikeReference(dDate, sReferenceTime) {
  if (!(dDate instanceof Date) || !Number.isFinite(dDate.getTime()))
    return String(sReferenceTime || "");
 ...
 * @returns {Array<{t: string, total: number, invested: number, pnl: number, y: number}>}
 */
export function fnEnsureInitialPortfolioJump(aPoints) {
  ...
  const oSyntheticPoint = {
    t: dBefore
      ? fnFormatTimeLikeReference(dBefore, oFirstPoint?.t)
      : `vor_${String(oFirstPoint?.t || "").trim()}`,
    total: 0,
    invested: 0,
    pnl: 0,
    y: 0,
 ...
 * @returns {Array<{t: string, total: number, invested: number, pnl: number, y: number}>}
 */
export function fnBuildZeroFallbackSeries(oQuery, nLastValue, sReferenceTime) {
  ...
  const iDefaultPointCount =
    nStepMs < 24 * 60 * 60 * 1000
  ...
  if (
    dStart instanceof Date &&
    dEnd instanceof Date &&
  ...
  for (let iIndex = 0; iIndex < iPointCount; iIndex += 1) {
    ...
    const sTimeValue = (() => {
      ...
      if (String(sReferenceTime || "").includes(" "))
        return fnFormatTimeLikeReference(dCurrent, sReferenceTime);
 ...
 * @returns {string}
 */
export function fnShortLabel(sValue, bIncludeDate = false) {
 ...
 * @returns {string}
 */
export function fnFmtAxisMoneyCompact(nValue, bPnlOnly) {
 ...
 * @returns {string}
 */
export function fnEscapeHtml(sInput) {

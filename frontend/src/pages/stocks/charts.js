...
export function fnEnsureLineChartHoverHandlers(elCanvas) {
  ...
  const fnResolveHoverIndex = (oEvent) => {
    ...
    const iHeight = Math.max(
      1,
      Math.round(oRect.height || elCanvas.clientHeight || elCanvas.height)
    );
...
export function fnDrawLineChart(oCtx, elCanvas, aPoints, bPnlOnly) {
  ...
  const sPrimary =
    oStyles.getPropertyValue("--clr-chart-line").trim() ||
    oStyles.getPropertyValue("--accent-blue").trim() ||
    oStyles.getPropertyValue("--clr-primary").trim() ||
    "#f39a67";
  ...
  const { bCompact, iPadL, iPadT, iInnerWidth, iInnerHeight } = fnGetLineChartGeometry(
    iWidth,
    iHeight
  );
  ...
  if (iHoverIndex >= 0 && iHoverIndex < aCanvasPoints.length) {
    ...
    const sHoverValue = Number.isFinite(nHoverValue)
      ? `${sHoverPrefix}${fnFmtMoney(nHoverValue, "USD")}`
      : "—";
...
export function fnDrawPieChart(oCtx, elCanvas, aItems, oOptions = {}) {
  ...
  const nSelectScale = Number.isFinite(oOptions.nSelectScale)
    ? Math.max(0, Math.min(1, oOptions.nSelectScale))
    : 0;

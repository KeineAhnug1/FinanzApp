 ...
 * @returns {string}
 */
function fnBuildDevelopmentCellHtml(
  oRow,
  mCurrentPriceBySymbol,
  sDisplayMode = "absolute",
  mRangeDevelopmentBySymbol = null
) {
  const sSymbol = String(oRow?.sSymbol || "")
    .trim()
    .toUpperCase();
  let nDevelopmentAbs;
  let nDevelopmentPct;

  const oRangeDevelopment =
    mRangeDevelopmentBySymbol instanceof Map ? mRangeDevelopmentBySymbol.get(sSymbol) : null;
  if (mRangeDevelopmentBySymbol instanceof Map) {
    ...
    nDevelopmentAbs =
      Number.isFinite(nCurrentPrice) && Number.isFinite(nAvgBuyPrice)
  ...
  const sText =
    sDisplayMode === "percent"
      ? fnFmtSignedPct(nDevelopmentPct)
      : fnFmtSignedMoney(nDevelopmentAbs);
...
function fnBuildRangeDevelopmentBySymbol(aRows, aPositions, mSeriesBySymbol, oQuery, sRange) {
  ...
  for (const oRow of aRows || []) {
    ...
    const aSymbolRawPoints = fnBuildSeriesPointsForPositions(
      aSymbolPositions,
      mSeriesBySymbol,
      false
    );
    ...
    for (const oPosition of aSymbolPositions) {
      const aPositionRawPoints = fnBuildSeriesPointsForPositions(
        [oPosition],
        mSeriesBySymbol,
        false
      );
      ...
      const bPurchasedInRange =
        Number.isFinite(iPurchaseMs) &&
...
export async function fnInitDepotView() {
  ...
  const fnRenderProfitLoss = () => {
    ...
    const sValueText = bShowPercent
      ? fnFmtSignedPct(nProfitLossPct)
      : fnFmtSignedMoney(nProfitLossAbs);
    const sClassName =
      nProfitLossAbs > 0
        ? "stock-performance--positive"
        : nProfitLossAbs < 0
          ? "stock-performance--negative"
          : "stock-performance--neutral";
    elKProfitLoss.classList.remove(
      "stock-performance--positive",
      "stock-performance--negative",
      "stock-performance--neutral"
    );
  ...
  const fnRenderDepotRows = () => {
    elHoldingsTbody.innerHTML = aDepotRows
      .map((oRow) => {
        ...
        const sBuyDate = Number.isFinite(oRow?.iEarliestBuyMs)
  ...
  elHoldingsTbody.addEventListener("click", (oEvent) => {
    const elTarget =
      oEvent.target instanceof Element ? oEvent.target.closest("td.stock-performance-cell") : null;
    if (!elTarget) return;
    ...
    try {
      await fnBuildDepotChart({
        ...
        fnOnDevelopmentBySymbolChange: (mNextDevelopmentBySymbol) => {
          mRangeDevelopmentBySymbol =
            mNextDevelopmentBySymbol instanceof Map ? mNextDevelopmentBySymbol : new Map();
          fnRenderDepotRows();
        },
      });
    } finally {
...
async function fnBuildDepotChart(oArgs) {
  ...
  const aDepotRows = Array.isArray(oArgs.aDepotRows)
    ? oArgs.aDepotRows
    : fnAggregateOwnedSymbols(aPositions);
  ...
  if (mSeriesBySymbol.size === 0) {
    oArgs.elInfo.textContent = fnT(
      "stocks.no_data_received",
      "Keine Kursdaten erhalten (API Limit? falsche Symbole?)."
    );
    fnClearCanvas(oArgs.oCtx, oArgs.elCanvas);
    oArgs.fnOnProfitLossChange?.(Number.NaN, Number.NaN);
    oArgs.fnOnCompositionChange?.([]);
  ...
  const mDevelopmentBySymbol = fnBuildRangeDevelopmentBySymbol(
    aDepotRows,
    aPositions,
    mSeriesBySymbol,
    oQuery,
  ...
  const aRawDisplayPoints = oArgs.bPnlOnly
    ...
    : aRawTotalPoints;
  const aDisplayPointsWithRange = fnWithFixedDateRange(
    aRawDisplayPoints,
    oQuery,
    oArgs.bPnlOnly,
    oArgs.sRange
  ...
  if (aTotalPoints.length === 0 || aDisplayPoints.length === 0) {
    ...
    const nCurrentTotal = aCurrentComposition.reduce(
      (nSum, oRow) => nSum + Number(oRow?.nValue || 0),
      0
    );
    ...
    if (aDisplayPoints.length === 0) {
      ...
      const nLastDisplayValue = oArgs.bPnlOnly
        ? nCurrentTotal - nInvestedTotalFallback
        : nCurrentTotal;
  ...
  oArgs.elKTotalLabel.textContent = fnT(
    "stocks.depot_value_last_point",
    "Depotwert (letzter Punkt)"
  );
  ...
  const nProfitLossAbs =
    Number.isFinite(nFirstDisplay) && Number.isFinite(nLastDisplay)
      ? nLastDisplay - nFirstDisplay
      : Number.NaN;
  const nProfitLossPct =
    Number.isFinite(nFirstDisplay) && nFirstDisplay > 0
...
export async function fnInitAnalysisView() {
  ...
  if (
    !elInfo ||
    !elCatalogSearchInput ||
    !elExchangeSelect ||
    !elAssetClassSelect ||
    !elCatalogSelectFirstBtn ||
    !elSearchResults ||
    !elOwnedSymbolSelect ||
    !elUseOwnedBtn ||
    !elTradeAmountInput ||
    !elTradeShareAccountSelect ||
    !elBuyBtn ||
    !elSellBtn ||
    !elBuyFeedback ||
    !elTableTbody ||
    ...
    fnShowError(
      fnT(
        "stocks.analysis_view_init_failed",
        "Einzelanalyse konnte nicht korrekt initialisiert werden (fehlende DOM-Elemente)."
      )
    );
    return;
  }
  ...
  let sSelectedTradeShareAccountId = fnNormalizeAccountId(
    sSelectedShareAccountId || aShareAccounts[0]?.id
  );
  let sSelectedExchange =
    typeof fnGetTradingExchange === "function" ? fnGetTradingExchange() : "NASDAQ";
  ...
  const fnRenderOwnedSelectOptions = () => {
    ...
    elOwnedSymbolSelect.innerHTML = aOwnedRows
      .map(
        (oRow) =>
          `<option value="${fnEscapeHtml(oRow.sSymbol)}">${fnEscapeHtml(oRow.sSymbol)}</option>`
      )
      .join("");
  ...
  const fnRenderTradeShareAccountOptions = () => {
    elTradeShareAccountSelect.innerHTML = aShareAccounts
      .map(
        (oAccount) =>
          `<option value="${fnEscapeHtml(oAccount.id)}">${fnEscapeHtml(oAccount.label)}</option>`
      )
      .join("");
    if (aShareAccounts.some((oAccount) => oAccount.id === sSelectedTradeShareAccountId)) {
      ...
      sSelectedTradeShareAccountId = fnNormalizeAccountId(
        sSelectedShareAccountId || aShareAccounts[0]?.id
      );
      elTradeShareAccountSelect.value = sSelectedTradeShareAccountId;
    }
  ...
  const fnRenderExchangeOptions = () => {
    const aExchanges =
      typeof fnGetCommonTradingExchanges === "function"
        ? fnGetCommonTradingExchanges()
        : [
            "XSTU",
            "XDUS",
    ...
    const sNormalizedSelectedExchange = String(
      sSelectedExchange === sNoExchangeValue
        ? sNoExchangeValue
        : typeof fnSetTradingExchange === "function"
          ? fnSetTradingExchange(sSelectedExchange)
          : sSelectedExchange
    ).toUpperCase();
    ...
    elExchangeSelect.innerHTML = [
      ...
      ...aExchanges.map((sExchange) => {
        const sNormalizedExchange = String(sExchange || "")
          .trim()
          .toUpperCase();
    ...
    elExchangeSelect.value =
      sNormalizedSelectedExchange === sNoExchangeValue ||
      aExchanges.includes(sNormalizedSelectedExchange)
        ? sNormalizedSelectedExchange
        : aExchanges[0];
    sSelectedExchange = String(elExchangeSelect.value || "")
      .trim()
      .toUpperCase();
  };
  ...
  const fnRenderAssetClassOptions = () => {
    ...
    elAssetClassSelect.value = ["stock", "etf", sNoAssetClassValue].includes(sSelectedAssetClass)
      ? sSelectedAssetClass
      : sNoAssetClassValue;
    sSelectedAssetClass = String(elAssetClassSelect.value || sNoAssetClassValue)
      .trim()
      .toLowerCase();
  };
  ...
  const fnRenderSearchResults = async () => {
    ...
    aCurrentSearchResults = await fnSearchStocksViaBackend(sNeedle, {
      sExchange: sSelectedExchange,
      sAssetClass: sSelectedAssetClass,
      iLimit: 20,
    });
  ...
  const fnGetLatestPriceBySymbol = async (sSymbol) => {
    try {
      const oQuoteResult = await fnGetLatestPriceBySymbolWithFallback(sSymbol);
      if (Number.isFinite(oQuoteResult?.nPrice) && oQuoteResult.nPrice > 0)
        return oQuoteResult.nPrice;
      return Number.NaN;
    } catch {
      return Number.NaN;
    }
  ...
  const fnRefreshAnalysisChart = async () => {
    ...
    try {
      ...
      nLastKnownClose = Number.isFinite(oChartResult?.nLastClose)
        ? oChartResult.nLastClose
        : Number.NaN;
    } finally {
  ...
  elCatalogSelectFirstBtn.addEventListener("click", async () => {
    ...
    if (!oFirstHit?.sSymbol) {
      elInfo.textContent = fnT(
        "stocks.no_selectable_hit",
        "Kein Treffer zur aktuellen Suche auswählbar."
      );
      return;
    }
  ...
  elSearchResults.addEventListener("click", async (oEvent) => {
    ...
    sSelectedSymbol = String(elTargetButton.dataset.symbol || "")
      .trim()
      .toUpperCase();
    elBuyFeedback.textContent = "";
    await fnRefreshAnalysisChart();
  ...
  elUseOwnedBtn.addEventListener("click", async () => {
    sSelectedSymbol = String(elOwnedSymbolSelect.value || "")
      .trim()
      .toUpperCase();
    elBuyFeedback.textContent = "";
    await fnRefreshAnalysisChart();
  ...
  elBuyBtn.addEventListener("click", async () => {
    ...
    try {
      const sSymbol = String(sSelectedSymbol || "")
        .trim()
        .toUpperCase();
      const nAmount = Number(elTradeAmountInput.value);

      if (!sSymbol) {
        elBuyFeedback.textContent = fnT(
          "stocks.select_stock_first",
          "Bitte zuerst eine Aktie auswählen."
        );
        return;
      }
      ...
      if (!Number.isFinite(nAmount) || nAmount <= 0) {
        elBuyFeedback.textContent = fnT(
          "stocks.invalid_buy_amount",
          "Bitte eine gültige Kaufmenge größer als 0 eingeben."
        );
        return;
      }
      if (nAmount > 1_000_000) {
        elBuyFeedback.textContent = fnT(
          "stocks.buy_amount_too_large",
          "Kaufmenge darf 1.000.000 nicht überschreiten."
        );
        return;
      }
      ...
      if (!sTargetShareAccountId) {
        elBuyFeedback.textContent = fnT(
          "stocks.buy_missing_share_account",
          "Kauf nicht möglich: Bitte ein Aktienkonto auswählen."
        );
        return;
      }
      ...
      if (!Number.isFinite(nBuyPrice) || nBuyPrice <= 0) {
        elBuyFeedback.textContent = fnT(
          "stocks.buy_price_unavailable",
          "Kauf nicht möglich: aktueller Kurs konnte nicht geladen werden."
        );
        return;
      }
      ...
      if (
        fnIsAllAccountsSelected() ||
        fnNormalizeAccountId(sSelectedShareAccountId) === sTargetShareAccountId
      ) {
      ...
      elBuyFeedback.textContent = fnT(
        "stocks.buy_saved_detail",
        "Kauf gespeichert: {amount} x {symbol} zu {price} auf {shareAccount}, abgebucht von {bankAccount}.",
        {
  ...
  elSellBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (!Number.isFinite(nAmount) || nAmount <= 0) {
        elBuyFeedback.textContent = fnT(
          "stocks.invalid_sell_amount",
          "Bitte eine gültige Verkaufsmenge größer als 0 eingeben."
        );
        return;
      }
      if (nAmount > 1_000_000) {
        elBuyFeedback.textContent = fnT(
          "stocks.sell_amount_too_large",
          "Verkaufsmenge darf 1.000.000 nicht überschreiten."
        );
        return;
      }
      ...
      if (!Number.isFinite(nOwnedAmount) || nOwnedAmount <= 0) {
        elBuyFeedback.textContent = fnT(
          "stocks.sell_not_possible_not_owned",
          "Verkauf nicht möglich: {symbol} ist nicht im Bestand.",
          { symbol: sSymbol }
        );
        return;
      }
      if (nAmount > nOwnedAmount) {
        elBuyFeedback.textContent = fnT(
          "stocks.sell_not_possible_stock",
          "Verkauf nicht möglich: Bestand {amount} {symbol}.",
          {
      ...
      if (!sTargetShareAccountId) {
        elBuyFeedback.textContent = fnT(
          "stocks.sell_missing_share_account",
          "Verkauf nicht möglich: Bitte ein Aktienkonto auswählen."
        );
        return;
      }
      ...
      if (nAmount > nOwnedAmountOnTargetAccount) {
        elBuyFeedback.textContent = fnT(
          "stocks.sell_not_possible_account_stock",
          "Verkauf nicht möglich: Bestand auf {account} ist {amount} {symbol}.",
          {
      ...
      if (!Number.isFinite(nSellPrice) || nSellPrice <= 0) {
        elBuyFeedback.textContent = fnT(
          "stocks.sell_price_unavailable",
          "Verkauf nicht möglich: aktueller Kurs konnte nicht geladen werden."
        );
        return;
      }
      ...
      elBuyFeedback.textContent = fnT(
        "stocks.sell_saved_detail",
        "Verkauf gespeichert: {amount} x {symbol} von {shareAccount}, gutgeschrieben auf {bankAccount}.",
        {
          amount: fnFmtNumber(nAmount, 4),
          symbol: sSymbol,
  ...
  if (sSelectedSymbol) {
    ...
    elInfo.textContent = fnT(
      "stocks.no_stock_available_use_search",
      "Keine Aktie verfügbar. Bitte Suche verwenden."
    );
    fnClearCanvas(oCtx, elCanvas);
  }
}
...
async function fnBuildAnalysisChart(oArgs) {
  const sSymbol = String(oArgs.sSelectedSymbol || "")
    .trim()
    .toUpperCase();
  if (!sSymbol) {
  ...
  if (!mSeriesBySymbol.size) {
    try {
      ...
      if (!Number.isFinite(nLast) || nLast <= 0)
        throw new Error(fnT("stocks.invalid_quote_price", "Quote ohne gueltigen Kurs."));
      ...
      oArgs.elTotalLabel.textContent = fnT(
        "stocks.price_symbol_last_point",
        "Kurs {symbol} (letzter Punkt)",
        { symbol: sSymbol }
      );
      ...
      oArgs.elInfo.textContent = fnT(
        "stocks.no_history_live_quote_fallback",
        "Keine Historie fuer {symbol}; Fallback auf Live-Quote ({source}{cache}).",
        {
          symbol: sSymbol,
      ...
      oArgs.elInfo.textContent = sDetail
        ? fnT(
            "stocks.no_data_for_symbol_with_detail",
            "Keine Kursdaten fuer {symbol}. Detail: {detail}",
            { symbol: sSymbol, detail: sDetail }
          )
        : fnT("stocks.no_data_for_symbol", "Keine Kursdaten fuer {symbol} erhalten.", {
            symbol: sSymbol,
          });
      fnClearCanvas(oArgs.oCtx, oArgs.elCanvas);
      return;
  ...
  if (aPoints.length === 0) {
    ...
    aPoints = fnBuildZeroFallbackSeries(
      oQuery,
      Number.isFinite(nLastValue) ? nLastValue : 0,
      sReferenceTime
    );
  }
  ...
  const sOwnedHint =
    nOwnedAmount > 0
      ? fnT("stocks.owned_hint", ", Bestand: {amount} Stk", {
          amount: fnFmtNumber(nOwnedAmount, 4),
        })
      : fnT("stocks.not_in_depot", ", nicht im Depot");
  oArgs.elInfo.textContent = fnT(
    "stocks.chart_updated_detail",
    "Chart aktualisiert: {symbol}, {range}{cache}{owned}",
    {
      symbol: sSymbol,
      range: oArgs.sRange,
...
function fnIsCatalogRowInCategory(oCatalogRow, sCategoryView) {
  ...
  const sType = String(oCatalogRow?.sType || "")
    .trim()
    .toLowerCase();
  const sName = String(oCatalogRow?.sName || "")
    .trim()
    .toLowerCase();
  ...
  if (sCategoryView === "category_etf") {
    return (
      sType.includes("etf") || sType.includes("exchange-traded fund") || sName.includes(" etf")
    );
  }
  ...
  if (sCategoryView === "category_depositary_receipts") {
    return (
      ...
      sType === "american depositary receipt" ||
      sType === "global depositary receipt"
    );
  }
...
export async function fnInitCategoryView(sCategoryView) {
  ...
  if (!elInfo || !elTableTbody) {
    fnShowError(
      fnT(
        "stocks.category_view_init_failed",
        "Kategorieansicht konnte nicht initialisiert werden (fehlende DOM-Elemente)."
      )
    );
    return;
  }
  ...
  try {
    [aPositions, aAllStocksCatalog] = await Promise.all([
      fnLoadPositions(),
      fnLoadAllStocksCatalog(),
    ]);
  } catch (oError) {
  ...
  if (!aFilteredRows.length) {
    elInfo.textContent = fnT(
      "stocks.no_owned_in_category",
      "Keine gehaltenen Aktien in dieser Kategorie gefunden."
    );
    elTableTbody.innerHTML = `
      <tr>
  ...
  elInfo.textContent = fnT(
    "stocks.symbol_count_in_category",
    "{count} Symbol(e) in dieser Kategorie gefunden.",
    { count: aFilteredRows.length }
  );
...
export function fnAskTransferTargetPrompt(aOptions) {
  ...
  const sList = aValidOptions
    .map((oOption, iIndex) => `${iIndex + 1}: ${oOption.label} (${oOption.id})`)
    .join("\n");
...
export async function fnInitAccountsView() {
  ...
  if (
    !elShareAccountNameInput ||
    !elCreateShareAccountBtn ||
    !elShareAccountsList ||
    !elBankAccountNameInput ||
    !elCreateBankAccountBtn ||
    !elBankAccountsList ||
    !elFeedback
  ) {
    return;
  ...
  const fnRefresh = async () => {
    ...
    fnRenderAccountList(
      elShareAccountsList,
      aShareAccounts,
      fnT("accounts.share_account_fallback", "Aktienkonto")
    );
    fnRenderAccountList(
      elBankAccountsList,
      aBankAccounts,
      fnT("accounts.bank_account_fallback", "Bankkonto")
    );
  };
  ...
  elCreateShareAccountBtn.addEventListener("click", async () => {
    ...
    if (!sLabel) {
      elFeedback.textContent = fnT(
        "accounts.enter_share_name",
        "Bitte einen Namen für das Aktienkonto eingeben."
      );
      return;
    }
    try {
      ...
      elFeedback.textContent = fnT(
        "accounts.share_create_failed",
        "Aktienkonto konnte nicht erstellt werden: {error}",
        { error: String(oError?.message || oError) }
      );
    }
  });
  ...
  elCreateBankAccountBtn.addEventListener("click", async () => {
    ...
    if (!sLabel) {
      elFeedback.textContent = fnT(
        "accounts.enter_bank_name",
        "Bitte einen Namen für das Bankkonto eingeben."
      );
      return;
    }
    try {
      ...
      elFeedback.textContent = fnT(
        "accounts.bank_create_failed",
        "Bankkonto konnte nicht erstellt werden: {error}",
        { error: String(oError?.message || oError) }
      );
    }
  });
  ...
  elShareAccountsList.addEventListener("click", async (oEvent) => {
    const elButton =
      oEvent.target instanceof Element ? oEvent.target.closest("button[data-action]") : null;
    if (!elButton) return;
    ...
    try {
      if (sAction === "rename") {
        ...
        try {
          ...
          if (oDetails?.requires_transfer) {
            ...
            elFeedback.textContent = fnT(
              "accounts.share_deleted_with_transfer",
              "Aktienkonto gelöscht und Shares übertragen."
            );
          } else {
            throw oError;
          }
      ...
      elFeedback.textContent = fnT("accounts.action_failed", "Aktion fehlgeschlagen: {error}", {
        error: String(oError?.message || oError),
      });
    }
  });

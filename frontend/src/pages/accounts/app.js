...
function fnMapAccounts(aRaw, sFallbackPrefix, oOptions = {}) {
  ...
  return (Array.isArray(aRaw) ? aRaw : [])
    .map((oAccount, iIndex) => {
      ...
      if (!sId) return null;
      const sLabel =
        String(oAccount?.label || oAccount?.name || "").trim() ||
        `${sFallbackPrefix} ${iIndex + 1}`;
...
async function fnLoadShareAccounts() {
  ...
  aShareAccounts = fnMapAccounts(
    oData?.accounts,
    t("accounts.share_account_fallback", "Aktienkonto"),
    { withBalance: false }
  );
}
...
function fnAskTransferTargetModal(aOptions, sMessage = "") {
  ...
  elSelect.innerHTML = aValidOptions
    .map(
      (oOption) =>
        `<option value="${fnEscapeHtml(oOption.id)}">${fnEscapeHtml(oOption.label)} (${fnEscapeHtml(fnFormatMoney(oOption.balance))})</option>`
    )
    .join("");
...
async function fnOpenAccountDetail(sAccountId, sType) {
  ...
  elBackdrop.addEventListener(
    "click",
    (e) => {
      if (e.target === elBackdrop) onClose();
    },
    { once: true }
  );
  ...
  try {
    if (isShare) {
      const aData = await fnApiRequest([
        `/api/positions?share_account_id=${encodeURIComponent(sAccountId)}`,
      ]);
...
async function fnInit() {
  ...
  elShareList.addEventListener("keydown", async (oEvent) => {
    ...
    const elInput =
      oEvent.target instanceof Element ? oEvent.target.closest(".account-name-input") : null;
    if (!elInput) return;
    ...
    if (oEvent.key === "Escape") {
      ...
      if (elSaveBtn) {
        elSaveBtn.textContent = t("accounts.rename", "Umbenennen");
        elSaveBtn.dataset.action = "rename";
      }
      return;
    }
    ...
    if (!sLabel) {
      elFeedback.textContent = t(
        "accounts.enter_valid_name",
        "Bitte einen gültigen Kontonamen eingeben."
      );
      return;
    }
    try {
      ...
      elFeedback.textContent = t("accounts.action_failed", "Aktion fehlgeschlagen: {error}", {
        error: String(oError?.message || oError),
      });
    }
  });
        ...
        try {
          ...
          if (oDetails?.requires_transfer) {
            const sTransferTargetId = await fnAskTransferTargetModal(
              ...
              t(
                "accounts.share_transfer_required",
                "Dieses Aktienkonto enthält noch Shares. Bitte ein Zielkonto wählen."
              )
            );
      ...
      if (sAction === "save") {
        ...
        try {
          ...
          if (oDetails?.requires_transfer) {
            ...
            elFeedback.textContent = t(
              "accounts.bank_deleted_with_transfer",
              "Bankkonto gelöscht und Guthaben übertragen."
            );
          } else {
            throw oError;
          }
  ...
  try {
    ...
    elFeedback.textContent = t(
      "accounts.load_failed",
      "Konten konnten nicht geladen werden: {error}",
      { error: String(oError?.message || oError) }
    );
  }
  ...
  document.addEventListener("click", async (oEvent) => {
    const elBtn =
      oEvent.target instanceof Element ? oEvent.target.closest("[data-view-account]") : null;
    if (!elBtn) return;

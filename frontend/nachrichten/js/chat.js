...
(function initChat() {
  ...
  async function loadMessages(partnerId, isPolling = false) {
    const result = await window.FinanzAppApi.requestJson(`/api/messages/conversation/${encodeURIComponent(partnerId)}`);
    if (!result.ok) {
  ...
  async function sendMessage(partnerId, content) {
    setStatus("");
    const result = await window.FinanzAppApi.requestJson("/api/messages/send", {
      method: "POST",
